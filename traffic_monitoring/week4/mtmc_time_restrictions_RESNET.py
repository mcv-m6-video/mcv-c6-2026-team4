...
SEQUENCE = "S04"
AI_CITY_SEQ_PATH = f"{AI_CITY_BASE_PATH}/train/" + SEQUENCE 
...
CAM_TIMESTAMP_FILE = os.path.join(AI_CITY_BASE_PATH, "cam_timestamp/" + SEQUENCE + ".txt")
...
if SEQUENCE == "S01":
    ...
    CAMERAS = ["c016", "c017", "c018", "c019", "c020", "c021", "c022", "c023", "c024", "c025", "c026"
                , "c027", "c028", "c029", "c030", "c031", "c032", "c033", "c034", "c035", "c036"
                , "c037", "c038", "c039", "c040"]

# Hyperparameters
CONF_THRESHOLD = 0.6543
IOU_THRESHOLD = 0.4968
MAX_AGE = 12              
NUM_CROPS_PER_TRACK = 5  
GLOBAL_MATCH_THRESH = 0.6832 
CAR_CLASS = 0
# -----------------------------------

# Time Constraints Configuration
APPLY_TIME_RESTRICTIONS = False       # Modify these three global variables, dont touch the other ones below in this section, they are automatically assigned.
...
SAVE_COLOR_DEBUG = True
COLOR_DEBUG_PATH = "./color_debug/" + SEQUENCE + "/roof_region"
# -----------------------------------
...
# Save YOLO predictions to json
SAVE_JSON = False
JSON_PATH = "/home-local/gasbert/master/C6/mcv-c6-2026-team4/traffic_monitoring/week4/detection_jsons/" + SEQUENCE
# -----------------------------------
...
def extract_reid_features(reid_model, transform, track_history, device, num_crops_per_track):
...
def extract_color_histogram(track_history, track_id=None, cam=None):
    """Compute HSV histogram using center crop to reduce background noise."""
    if len(track_history) == 0:
        return None

    if SAVE_COLOR_DEBUG:
        os.makedirs(COLOR_DEBUG_PATH, exist_ok=True)

    hists = []

    for i, entry in enumerate(track_history):
        crop = entry["img"]
        ...
        selected_region = crop[y1:y2, x1:x2]

        if selected_region.size == 0:
            continue

        hsv = cv2.cvtColor(selected_region, cv2.COLOR_BGR2HSV)
        ...
        if SAVE_COLOR_DEBUG:
            ...
            # Draw rectangle (selected region)
            cv2.rectangle(debug_img, (x1, y1), (x2, y2), (0, 255, 0), 2)

            # Optional: darken outside region
            mask = np.zeros_like(debug_img, dtype=np.uint8)
            mask[y1:y2, x1:x2] = debug_img[y1:y2, x1:x2]
            debug_overlay = cv2.addWeighted(debug_img, 0.4, mask, 0.6, 0)

            filename = f"{cam}_track{track_id}_frame{frame_id}_{i}.jpg"
            save_path = os.path.join(COLOR_DEBUG_PATH, filename)

            cv2.imwrite(save_path, debug_overlay)
...
def main():
    ...
    num_crops_per_track = NUM_CROPS_PER_TRACK if not WANDB else config.num_crops_per_track
    max_time_between_cameras = MAX_TIME_BETWEEN_CAMERAS  # if not WANDB else config.max_time_between_cameras
    ...
    for cam in CAMERAS:
        ...
        for idx, frame in enumerate(tqdm(video, desc="Tracking & Cropping")):
            ...
            yolo_detections_per_frame.append(frame_detections)
        ...
        if SAVE_JSON:
            ...
            json_output_path = os.path.join(JSON_PATH, f"{cam}_detections.json")
            with open(json_output_path, "w") as f:
                json.dump(yolo_detections_per_frame, f, indent=4)

            print(f"Saved YOLO detections to {json_output_path}")

        # Step B: Feature Extraction & Global Matching
        for track in tqdm(finished_tracks, desc="Re-ID & Global Linking"):
            signature = extract_reid_features(reid_model, reid_transform, track["history"], device, num_crops_per_track)
    ...
    summary = aicity_eval(test_df, pred_df, mread=False, dstype="train/"+SEQUENCE, roidir=AI_CITY_BASE_PATH)
