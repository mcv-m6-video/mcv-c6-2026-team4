...
from src.models.opencv_models import MOG2BackgroundSubtractor
...
def objective(trial: optuna.Trial, annotations: Dict[int, list[BoundingBox]],
    ...
    var_threshold = trial.suggest_float("var_threshold", 4.0, 64.0)
    detect_shadows = trial.suggest_categorical("detect_shadows", [True, False])
    ...
    model = MOG2BackgroundSubtractor(
        history=history,
...
def main():
    parser = argparse.ArgumentParser(description="Optuna hyperparameter search for MOG2 model")
    parser.add_argument("--n-trials", type=int, default=50, help="Number of trials to run")
    parser.add_argument("--study-name", type=str, default="mog2_model_optimization",
                       help="Name of the Optuna study")
    parser.add_argument("--db-path", type=str, default="optuna_mog2_model.db",
