"""
Plot results from mean_rho parameter study

Usage:
    # Plot from a single results file
    python plot_mean_rho_results.py --input adaptive_ro_results/mean_rho_A.json --output_dir mean_rho_plots

    # Plot from multiple results files (if you ran in parallel)
    python plot_mean_rho_results.py --input adaptive_ro_results/mean_rho_*.json --output_dir mean_rho_plots
"""
...
def load_results(input_files):
    ...
    # Sort by mean_rho value
    all_results.sort(key=lambda x: x['mean_rho'])

    return all_results


def extract_metric_arrays(results, metric_name):
    """Extract arrays of mean_rho values and corresponding metric values"""
    mean_rhos = [r['mean_rho'] for r in results]
    total_metrics = [r['total_metrics'][metric_name] for r in results]

    return np.array(mean_rhos), np.array(total_metrics)
...
def extract_per_frame_data(results, metric_name):
    ...
    # Create matrix: rows = mean_rhos, cols = frames
    data_matrix = np.full((len(mean_rhos), len(frame_ids)), np.nan)
    ...
    return mean_rhos, frame_ids, data_matrix


def plot_mean_rho_vs_total_metrics(results, output_dir):
    """Plot 1: mean_rho vs Total Metrics (AP, AP50, AP75, etc.)"""
    ...
    for idx, metric in enumerate(metrics_to_plot):
        mean_rhos, values = extract_metric_arrays(results, metric)

        axes[idx].plot(mean_rhos, values, 'o-', linewidth=2, markersize=8)
        axes[idx].set_xlabel('mean_rho', fontsize=18)
        axes[idx].set_ylabel(metric, fontsize=18)
        axes[idx].set_title(f'{metric} vs mean_rho', fontsize=22, fontweight='bold')
        axes[idx].tick_params(axis='both', which='major', labelsize=16)
        axes[idx].grid(True, alpha=0.3)

        # Mark best value
        best_idx = np.argmax(values)
        axes[idx].axvline(mean_rhos[best_idx], color='r', linestyle='--', alpha=0.5, label=f'Best: ρ={mean_rhos[best_idx]:.4f}')
        axes[idx].legend(fontsize=14)

    plt.tight_layout()
    plt.savefig(output_dir / 'mean_rho_vs_total_metrics.png', dpi=300, bbox_inches='tight')
    plt.close()
    print(f"Saved: mean_rho_vs_total_metrics.png")


def plot_heatmap(results, output_dir, metric_name='AP50'):
    """Plot 2: Heatmap of mean_rho vs Frame Number with metric as color"""
    mean_rhos, frame_ids, data_matrix = extract_per_frame_data(results, metric_name)
    ...
    ax.set_yticks(np.arange(len(mean_rhos)))
    ax.set_yticklabels([f'{r:.4f}' for r in mean_rhos])

    ax.set_xlabel('Frame ID', fontsize=12)
    ax.set_ylabel('mean_rho', fontsize=12)
    ax.set_title(f'{metric_name} Heatmap: mean_rho vs Frame', fontsize=14, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig(output_dir / f'heatmap_mean_rho_vs_frame_{metric_name}.png', dpi=300, bbox_inches='tight')
    plt.close()
    print(f"Saved: heatmap_mean_rho_vs_frame_{metric_name}.png")


def plot_per_frame_lines(results, output_dir, metric_name='AP50', max_rhos_to_plot=10):
    """Plot 3: Line plots showing metric per frame for different mean_rho values"""
    mean_rhos, frame_ids, data_matrix = extract_per_frame_data(results, metric_name)

    # If too many rhos, subsample for readability
    if len(mean_rhos) > max_rhos_to_plot:
        indices = np.linspace(0, len(mean_rhos)-1, max_rhos_to_plot, dtype=int)
        rhos_to_plot = [mean_rhos[i] for i in indices]
        data_to_plot = data_matrix[indices, :]
    else:
        rhos_to_plot = mean_rhos
    ...
    ax.set_title(f'{metric_name} per Frame for Different mean_rho Values', fontsize=14, fontweight='bold')
...
def print_summary_table(results):
    ...
    # Find best mean_rho for each metric
    best_ap = max(results, key=lambda x: x['total_metrics']['AP'])
    best_ap50 = max(results, key=lambda x: x['total_metrics']['AP50'])

    print(f"\nBest AP:    mean_rho = {best_ap['mean_rho']:.4f} (AP = {best_ap['total_metrics']['AP']:.4f})")
    print(f"Best AP50:  mean_rho = {best_ap50['mean_rho']:.4f} (AP50 = {best_ap50['total_metrics']['AP50']:.4f})")
    print()


def main():
    parser = argparse.ArgumentParser(description="Plot mean_rho parameter study results")
    parser.add_argument("--input", nargs='+', required=True, help="Input JSON file(s) with results (supports glob patterns)")
    parser.add_argument("--output_dir", type=str, default="mean_rho_plots", help="Output directory for plots")
    ...
    print(f"Loaded {len(results)} experiments with mean_rho values from {results[0]['mean_rho']:.4f} to {results[-1]['mean_rho']:.4f}")
    ...
    # Plot 1: mean_rho vs Total Metrics
    plot_mean_rho_vs_total_metrics(results, output_dir)
