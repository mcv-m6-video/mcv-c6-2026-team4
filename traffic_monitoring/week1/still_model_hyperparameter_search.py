...
def objective(trial: optuna.Trial, annotations: Dict[int, list[BoundingBox]],
    ...
    alpha = trial.suggest_float("alpha", 1.0, 7.0)
    ...
    aspect_ratio_min = trial.suggest_float("aspect_ratio_min", 0.1, 0.25)
    aspect_ratio_max = trial.suggest_float("aspect_ratio_max", 4.0, 15.0)
    fill_ratio_min = trial.suggest_float("fill_ratio_min", 0.1, 0.3)
    fill_ratio_max = 1.0 

    # Temporal hyperparameters
    if use_temporal:
        n_frames = trial.suggest_int("n_frames", 2, 3)
    ...
    model = GrayGaussianModel(alpha=alpha, std_bias=std_bias)
...
def main():
    parser = argparse.ArgumentParser(description="Optuna hyperparameter search for still model")
    parser.add_argument("--n-trials", type=int, default=50, help="Number of trials to run")
    parser.add_argument("--study-name", type=str, default="still_model_optimization",
                       help="Name of the Optuna study")
    parser.add_argument("--db-path", type=str, default="optuna_still_model.db",
