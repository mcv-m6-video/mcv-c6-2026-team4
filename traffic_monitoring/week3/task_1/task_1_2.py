...
# Import pyflow
pyflow_path = os.path.abspath(os.path.join(parent_dir, 'pyflow'))
...
def load_gt_with_tracks_txt(path: str):
    """Custom parser to extract ground truth bounding boxes from MOT16 format txt files."""
    gt_with_tracks = dict()
    ...
    with open(path, 'r') as f:
        for line in f:
            ...
            # Calculate right and bottom from width and height
            right = left + width
            bottom = top + height
    ...
    print(f"Loaded GT for {len(gt_with_tracks)} frames.") 
    return gt_with_tracks


def load_gt_with_tracks_xml(path: str):
    """Custom parser to extract ground truth bounding boxes ALONG with their track IDs."""
    tree = ET.parse(path)
    tracks = tree.findall("track")
    gt_with_tracks = dict()

    for track in tracks:
        # 1. Check the label at the track level (case-insensitive)
        label = track.get("label", "").lower()
        if label == "car" or label == "vehicle": 
            track_id = int(track.get("id"))
            
            # 2. Iterate through each box in this track
            for box in track.findall("box"):
                # 3. Check the 'outside' flag at the BOX level
                if box.get("outside") == "0":
                    ...
                    if frame not in gt_with_tracks:
                        gt_with_tracks[frame] = []
                    gt_with_tracks[frame].append((bbox, track_id))
...
class OpticalFlowTracker:
    ...
    def update(self, detections: list[BoundingBox], u: np.ndarray, v: np.ndarray, frame_idx: int, config):
        current_iou_thresh = config.iou_threshold if config else IoU_THRESHOLD
        current_max_age = config.max_age if config else MAX_AGE
        new_active_tracks = {}
...
def main():
    ...
    gt_with_tracks = load_gt_with_tracks_xml(XML_PATH)

    print("Loading YOLO Model...")
    model = YOLO(YOLO_WEIGHTS)
    
    print(f"Loading Video: {VIDEO_PATH}")
    video = VideoPartSource(VIDEO_PATH, start_frac=0, end_frac=1) 
    ...
    for idx, frame in enumerate(tqdm(video)):
        ...
        # YOLO Detection
        current_conf = config.conf_threshold if WANDB else CONF_THRESHOLD
    ...
    if SAVE_VIDEO:
        ...
        video_vis = VideoPartSource(VIDEO_PATH, start_frac=0, end_frac=1) 
